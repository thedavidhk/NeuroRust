...
impl Neuron {
    ...
    pub fn forward(&self, inputs: &[f64]) -> f64 {
        assert_eq!(inputs.len(), self.weights.len(), "Input size must match weights size.");
    ...
    #[cfg(test)]
    pub fn new_mock() -> Neuron {
        Neuron {weights: vec![1.0], bias: 0.0}
    }
}
...
mod tests {
    ...
    fn test_neuron_new() {
        ...
        assert_eq!(neuron.weights.len(), input_size, "Number of weights should match input size.");
        assert!(neuron.weights.iter().any(|&w| w != 0.0), "Weights should not be all zero.");
        assert_ne!(neuron.bias, 0.0, "Bias should be initialized.");
    }

    #[test]
    fn test_neuron_forward() {
        let neuron = Neuron { weights: vec![-0.5, 0.5], bias: 1.0 };
        let inputs = vec![1.0, 2.0];
        let expected_output = 1.5;
        let actual_output = neuron.forward(&inputs);
        assert!((actual_output - expected_output).abs() < 1e-5, "Forward function does not produce expected output.")
    }
}
